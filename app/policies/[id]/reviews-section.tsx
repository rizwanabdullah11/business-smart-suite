...
export default function ReviewsSection({ policyId, reviews, canEdit }: ReviewsSectionProps) {
  ...
  const handleDelete = async (reviewId: string) => {
    ...
    try {
      const response = await fetch(`/api/policies/${policyId}/reviews?reviewId=${reviewId}`, {
        method: "DELETE",
      })
  ...
  return (
    <div className="p-4">
      {canEdit && (
        <div className="mb-4">
          <AddReviewDialog policyId={policyId} />
        </div>
      )}
