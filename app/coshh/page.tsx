...
// Sample data for COSHH
const initialCategories = [
  {
    id: "1",
    title: "Cleaning Materials",
    assessments: [
      { id: "1-1", title: "Floor Cleaner Assessment", version: "v2.0", issueDate: "2024-01-10", location: "General", highlighted: true, approved: true, paused: false },
      { id: "1-2", title: "Glass Cleaner Safety Data", version: "v1.5", issueDate: "2024-02-15", location: "Office", highlighted: false, approved: true, paused: false },
    ]
  },
  {
    id: "2",
    ...
    assessments: [
      { id: "2-1", title: "Solvent A Handling", version: "v3.0", issueDate: "2024-03-05", location: "Production", highlighted: false, approved: true, paused: false },
      { id: "2-2", title: "Adhesive B Storage", version: "v1.1", issueDate: "2024-04-20", location: "Warehouse", highlighted: false, approved: false, paused: true },
    ]
  },
]
...
function LegacyCOSHHPage() {
  ...
  const [addingAssessmentToCategory, setAddingAssessmentToCategory] = useState<string | null>(null)
  ...
  const toggleHighlight = (categoryId: string, assessmentId: string) => {
    setCategories(prev =>
      prev.map(cat =>
        cat.id === categoryId
          ? {
            ...
            assessments: cat.assessments.map(a =>
              a.id === assessmentId ? { ...a, highlighted: !a.highlighted } : a
            )
          }
          : cat
      )
  ...
  const toggleApprove = (categoryId: string, assessmentId: string) => {
    setCategories(prev =>
      prev.map(cat =>
        cat.id === categoryId
          ? {
            ...cat,
            assessments: cat.assessments.map(a =>
              a.id === assessmentId ? { ...a, approved: !a.approved } : a
            )
          }
          : cat
      )
  ...
  const togglePause = (categoryId: string, assessmentId: string) => {
    setCategories(prev =>
      prev.map(cat =>
        cat.id === categoryId
          ? {
  ...
  const deleteAssessment = (categoryId: string, assessmentId: string) => {
    if (confirm("Are you sure you want to delete this COSHH assessment?")) {
      setCategories(prev =>
        prev.map(cat =>
          cat.id === categoryId
            ? {
              ...cat,
              assessments: cat.assessments.filter(a => a.id !== assessmentId)
            }
            : cat
        )
      )
  ...
  const addAssessmentToCategory = (categoryId: string) => {
    if (newAssessmentData.title.trim()) {
      const newAssessment = {
        id: `${categoryId}-${Date.now()}`,
        title: newAssessmentData.title.trim(),
        version: newAssessmentData.version.trim() || "v1.0",
        location: newAssessmentData.location.trim() || "N/A",
        content: newAssessmentData.content.trim() || "",
        issueDate: newAssessmentData.issueDate,
        highlighted: false,
        approved: false,
        paused: false
      }
      ...
      setCategories(prev =>
        prev.map(cat =>
          cat.id === categoryId
            ? { ...cat, assessments: [...cat.assessments, newAssessment] }
            : cat
        )
      )
      ...
      setAddingAssessmentToCategory(null)
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: COLORS.textSecondary }}>
                Manage Control of Substances Hazardous to Health
              </p>
            </div>
          </div>
                  ...
                  <div className="p-5">
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                          ...
                          <div>
                            ...
                            <input
                              type="text"
                              value={newAssessmentData.version}
                              onChange={(e) => setNewAssessmentData(prev => ({ ...prev, version: e.target.value }))}
                          ...
                          <div>
                            ...
                            <input
                              type="text"
                              value={newAssessmentData.location}
                              onChange={(e) => setNewAssessmentData(prev => ({ ...prev, location: e.target.value }))}
                        ...
                        <div className="mb-4">
                          ...
                          <textarea
                            value={newAssessmentData.content}
                            onChange={(e) => setNewAssessmentData(prev => ({ ...prev, content: e.target.value }))}
                        ...
                        <div className="flex gap-3">
                          <button
                            onClick={() => addAssessmentToCategory(category.id)}
                            className="px-6 py-2.5 rounded-lg font-medium hover:shadow-md transition-all"
                          ...
                          <button
                            onClick={() => setAddingAssessmentToCategory(null)}
                            className="px-6 py-2.5 rounded-lg font-medium hover:shadow-md transition-all"
                    ...
                    {sortedAssessments.length === 0 ? (
                      <div className="text-center py-12">
                        <FlaskConical className="w-12 h-12 mx-auto mb-3" style={{ color: COLORS.textLight }} />
                      ...
                      <div className="space-y-3">
                        {sortedAssessments.map((assessment) => (
                          <div
                            ...
                            style={{
                              background: assessment.paused ? `${COLORS.warning}05` : assessment.highlighted ? `${COLORS.primary}05` : COLORS.bgWhite,
                              border: `1px solid ${COLORS.border}`,
                            }}
                          >
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex gap-4 text-sm mt-1.5" style={{ color: COLORS.textSecondary }}>
                                <span className="flex items-center gap-1">
                                  <span className="font-medium">Version:</span> {assessment.version}
                                </span>
                                <span className="flex items-center gap-1">
                                  <Calendar className="w-3 h-3" />
                                  Issued: {assessment.issueDate}
                                </span>
                                <span className="flex items-center gap-1">
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <div className="flex items-center gap-1">
                                <Link href={`/coshh/${assessment.id}/edit`}><button className="p-2 rounded-lg hover:scale-105" style={{ background: "#DBEAFE", color: "#1E40AF" }}><Edit className="w-4 h-4" /></button></Link>
...
export default function COSHHPage() {
  return (
    <DynamicModulePage
      moduleSlug="coshh"
      title="COSHH"
      description="Manage COSHH assessments and records"
      itemLabel="Assessment"
      icon={FlaskConical}
      newItemHref="/coshh/new"
      itemHrefPrefix="/coshh"
    />
  )
}
